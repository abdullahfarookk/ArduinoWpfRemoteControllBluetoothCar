...
namespace BluetoothSample.Services
{
    ...
    public sealed class SenderBluetoothService : ISenderBluetoothService
    {
        ...
        public SenderBluetoothService()
        {
            ...
            _serviceClassId = new Guid("0000110a-0000-1000-8000-00805f9b34fb");
        ...
        public async Task<bool> Send(Device device, string content)
        {
            ...
            var task = Task.Run(() =>
            {
                using (var bluetoothClient = new BluetoothClient())
                {
                    try
                    {
                        device.DeviceInfo.SetServiceState(BluetoothService.SerialPort, true);
                        
                        var ep = new BluetoothEndPoint(device.DeviceInfo.DeviceAddress, BluetoothService.SerialPort);
